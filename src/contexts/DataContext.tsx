...
import { Product, Category, Order, StoreSettings, Review, PRODUCTS_DATA, CATEGORIES_DATA, ORDERS_DATA, STORE_SETTINGS_DATA, REVIEWS_DATA } from '../data/mock-data';
...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const addOrder = (order: Omit<Order, 'id' | 'stockDeducted'>): Order => {
    ...
    // Stock is no longer deducted on creation, but on confirmation
  ...
  const updateOrderPaymentStatus = (orderId: string, paymentStatus: Order['paymentStatus']) => {
    setOrders(prev => prev.map(o => {
      if (o.id === orderId) {
        ...
        // NOTE: We are not handling restocking on setting back to 'Unpaid' for simplicity.
        return updatedOrder;
      }
      return o;
    }));
