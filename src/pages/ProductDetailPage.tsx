                ...
                <div>
                    ...
                    <div className="grid grid-cols-5 gap-2">
                        {product.imageUrls.map((url, index) => (
                            <button key={index} onClick={() => setMainImage(url)} className={`aspect-square w-full rounded-md overflow-hidden border-2 transition-transform hover:scale-105 ${mainImage === url ? 'border-accent-teal' : 'border-transparent'}`}>
                            ...
                            <div className="flex items-center gap-2">
                                {product.colors.map(color => (
                                    <button 
                                        ...
                                        className={`h-8 w-8 rounded-full border-2 transition-transform duration-200 ${selectedColor === color ? 'border-accent-teal scale-110' : 'border-gray-300'}`}
                                ...
                                isReviewSubmitted ? (
                                    ...
                                    <form onSubmit={handleReviewSubmit} className="mt-4 space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <textarea id="comment" value={newReviewComment} onChange={e => setNewReviewComment(e.target.value)} rows={4} required className="mt-1 flex w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm ring-offset-white placeholder:text-gray-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-teal dark:bg-gray-900/50 dark:border-gray-600 dark:text-white dark:placeholder-gray-400" placeholder="What did you like or dislike?"/>
                                        </div>
